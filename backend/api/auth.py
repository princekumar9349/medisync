...
from core.audit import log_audit_event, extract_device_metadata
...
_CARETAKER_JWT_MINUTES = 30   # Reduced to 30-min for stateful caregiver session
...
def caretaker_login(payload: CaretakerLogin, request: Request):
    """
    Authenticate a family member/caretaker.

    - PIN is stored as a SHA-256 hash in the patient's user document
    - Issues a short-lived (30 min) JWT with role=caretaker
    - Stateful session generated in DB for immediate revocation
    - Rate-limited: 5 failed attempts → 5 minute lockout
    - Logs activity to patient document & audit log
    ...
    session_id = SessionManager.create_session(
        ...
        role="caretaker",
        ttl_minutes=_CARETAKER_JWT_MINUTES,
        device_id=device_id
    )
...
def set_caretaker_pin(
    ...
    SessionManager.revoke_all_sessions(current_user.user_id, role="caretaker")

    log_audit_event(
        action="caregiver_pin_updated",
        actor_id=current_user.user_id,
        actor_role=current_user.role or "patient",
...
def generate_caretaker_pin(
    ...
    log_audit_event(
        action="caregiver_pin_generated",
        actor_id=current_user.user_id,
        actor_role=current_user.role or "patient",
        target_id=current_user.user_id,
        session_id=current_user.session_id,
...
def revoke_caretaker_access(request: Request, current_user: TokenData = Depends(get_current_user)):
    """
...
def toggle_caretaker_access(
    ...
    log_audit_event(
        action=f"caregiver_access_toggle_{action}",
        actor_id=current_user.user_id,
        actor_role=current_user.role or "patient",
        target_id=current_user.user_id,
        session_id=current_user.session_id,
