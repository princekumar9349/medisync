...
def mark_done(
    ...
    # Invalidate analytics snapshot so next read triggers recompute
    try:
        from analytics.snapshots.manager import invalidate_user_snapshot
        invalidate_user_snapshot(current_user.user_id, reason="dose_logged")
    except Exception:
        pass  # Never let analytics code block the primary dose action
...
def patient_delete_medicine(
    ...
    # Invalidate analytics snapshot — schedule change affects adherence scores
    try:
        from analytics.snapshots.manager import invalidate_user_snapshot
        invalidate_user_snapshot(current_user.user_id, reason="medicine_deleted")
    except Exception:
        pass
